...
class VideoStitcher:
    def __init__(self, left_video_in_path, right_video_in_path, video_out_path, video_out_width=1400, display=False):
    ...
    def draw_matches(image_a, image_b, keypoints_a, keypoints_b, matches, status):
        ...
        visualisation = np.zeros((max(height_a, height_b), width_a + width_b, 3), dtype="uint32")
    ...
    def run(self):
        ...
        print('[INFO]')
        clip.write_videofile("stitch.mp4", audio=False)
        print('[INFO]: {} saved'.format(self.video_out_path.split('/')[-1]))


# Example call to 'VideoStitcher'
stitcher = VideoStitcher(left_video_in_path='C:/image/video/left.mp4',
                         right_video_in_path='C:/image/video/right.mp4',
                         video_out_path='C:/image/video/stich.mp4')
stitcher.run()
